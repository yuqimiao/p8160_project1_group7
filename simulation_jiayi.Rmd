...
simulation <- function(time, n, p, percentage, c = 1, mu = 1, sigma = 0.5, beta = NULL, seed = 100, error_sigma = 1, correlation = 0.5, type = "density", weak_corr_density = 0.5) {
  ...
  for (j in 1:time) {
    ...
    fitted_values = tibble(true_y = as.vector(response), 
    ...
    model_measures = beta_result %>% 
      select(- `lr coefficient`) %>% 
      rename(step = `step coefficient`, lasso=`lasso coefficient`) %>% 
      pivot_longer(step:lasso, names_to  = "method", values_to = "coefficient") %>% 
      mutate(correct = ifelse((type == "strong"& coefficient != 0) | 
                                     (type != "strong" & coefficient == 0), 1, 0 )) %>% 
      mutate(n = sample_size, sigma = sigma)
...
# store row beta results in a list
row_results <- list()
...
get_measures <- function(beta_df){
    # recall, precision, F1
    recall = beta_df %>% filter(type == "strong") %>% group_by(method, n, sigma) %>% summarize(recall = mean(correct))
    precision = beta_df %>% filter(correct == 1) %>% mutate(strong = type == "strong") %>% group_by(method, n, sigma) %>% summarize(precision = mean(strong))
    
    # output a tibble
    return(left_join(recall, precision, by = c("method", "n", "sigma")) %>% mutate(f1 = 2/(1/recall + 1/precision)))
}

measures_result = get_measures(row_results[[1]])
for(i in 2:length(row_results)){
  measures_result = rbind(measures_result, get_measures(row_results[[i]]))
}
measures_result$idx = idx
