...
simulation <- function(time, n, p, percentage, c = 1, mu = 1, sigma = 0.5, beta = NULL, seed = 100, error_sigma = 1, correlation = 0.5, type = "density", weak_corr_density = 0.5) {
  ...
  if (length(beta) == 0) {
    strong_beta = runif(as.integer(strong * 0.5* p), min = c * sqrt(log(p) / n), max = c * sqrt(p / n))
  ...
  for (j in 1:time) {
    ...
    beta_result = tibble(
        ...
        "step" = coefficients(step_lr)[2:length(coefficients(step_lr))],
    ...
    step_models[[j]] = step_lr
    lasso_models[[j]] = lasso
  }
  ...
  result = list(beta = beta_list, measures = measures, fitted = fitted, c = c, percentage = c(strong, weak_corr, weak_no_corr), cov_matrix = sigma_matrix * sigma, step_models = step_models, lasso_models = lasso_models)
  return(result)
}
...
# store all row results (including models, betas, predicted value etc.) in a list
row_results <- list()
for (sample_size in c(200, 1000, 5000)) {
  for (sigma in c(0.3, 0.8)) {
    temp_results <- simulation(time = 100, n = sample_size, p = 200, percentage = c(5, 3, 2), 
                           sigma = sigma)
    row_results <- append(row_results, temp_results)
  }
}
# saveRDS(row_results, file = "beta_estimates.rds")

    temp_results <- simulation(time = 100, n = 200, p = 200, percentage = c(5, 3, 2), 
                           sigma = 0.3)
```
...
get_measures <- function(beta_df){
    # recall, precision, F1
    recall = beta_df %>% filter(type != "null") %>% group_by(method, type,n, sigma) %>% summarize(recall = mean(correct))
    precision = beta_df %>% filter(correct == 1) %>% mutate(not_null = ifelse(type == "null",0,1)) %>% group_by(method, type,n, sigma) %>% summarize(precision = mean(not_null))
    
    # output a tibble
    return(left_join(recall, precision, by = c("method","type", "n", "sigma")) %>% 
             mutate(f1 = 2/(1/recall + 1/precision)))
}

measures_result = get_measures(row_results[[1]])
for(i in 2:length(row_results)){
  measures_result = rbind(measures_result, get_measures(row_results[[i]])) %>% mutate(idx = i)
}
# saveRDS(measures_result, file = "measurement_scores.rds")
```
...
for (sample_size in c(200, 1000, 5000)) {
  for (sigma in c(0.3, 0.8)) {
    temp_results <- simulation(time = 100, n = sample_size, p = 200, percentage = c(5, 3, 2), 
                           sigma = sigma)$fitted
    fitted_results <- append(fitted_results, temp_results)
  }
}
